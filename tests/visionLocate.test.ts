...
describe("an answer we can act on", () => {
  ...
  it("accepts an element flush against the edge of the screen", () => {
    // The window close button really does sit at the very corner, and really does come back
    // reported a pixel or two past it. Refusing here would be a silly way to be principled.
    const result = checkLocation(found({ x: 1560, y: -2, width: 12, height: 24 }), SHOT, "the close button");
    // Clamped back into the frame rather than refused.
    expect(result.box).toEqual({ x: 1560, y: 0, width: 8, height: 22 });
  });
});
...
describe("an answer we refuse to act on", () => {
  ...
  it("refuses a box too small to be a control", () => {
    // 7x7 = 49, under the 64 floor; 8x8 = 64 is exactly on it and allowed.
    expect(MIN_BOX_AREA).toBe(64);
    expect(refusal(() => checkLocation(found({ x: 10, y: 10, width: 7, height: 7 }), SHOT, "the dot")).refusal).toBe(
      "untrustworthy",
    );
    expect(checkLocation(found({ x: 10, y: 10, width: 8, height: 8 }), SHOT, "the dot").box).toEqual({
      x: 10,
      y: 10,
      width: 8,
      height: 8,
